...
if __name__ == "__main__":
    ...
    print(dataSet.head())
    dataFrame_origion = dataSet.groupby('label')
    ...
    # with open('./TREC_train_kg_balance_test.txt', 'w+') as fp:
    #     for each in file_list:
    #         fp.writelines(each + '\n')

    print("""分析KG后的句子""")
    with open('./TREC_train_kg_balance_test.txt') as fp:
        file_content = fp.readlines()
    label  = []
    belong = []
    for index,eachline in enumerate(file_content):
        label.append(eachline.split()[1])
        belong.append(eachline.split()[0])

    dataSet = pd.DataFrame()
    dataSet['content'] = file_content
    dataSet['label']   = label
    ...
    data_origion = pd.DataFrame()
    data_origion['content'] = file_list
    data_origion['label']   = label

    all_choice_data = bucket_balance(data_origion,dataSet,"label","label","belong")
